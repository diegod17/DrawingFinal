...
function loadImage(obj) {
  ...
  checkbox.setAttribute("class", "likeCls");
  checkbox2.setAttribute("class", "dislikeCls");
  ...
  figure.style.borderColor = obj.color;
 ...
 function checkValue(obj) {
        ...
        if (!(localSavedLikes.some(e => e.fileName == obj.name))) {
         ...
         if (checkbox1.checked && checkbox2.checked) {
          ////changeColorofTextSamewithotherones
          if (checkbox1.getAttribute('class') == "likeCls"){
              obj.likeCounter.style.color = "green";
          }
          else{
              obj.likeCounter.style.color = "red";
          }
                //change checkbox
            ...
            console.log("we do wrong code");

            obj.likeCounter.style.color = "black";
        ...
        function loadSavedLikes(e) {
            if(e.state != "n" && document.getElementById(e.fileName + e.state))
            {
                ...
                handle.checked = true;
                likeCounter = Number(handle.likeCounter.innerText)
                if(likeCounter>0)
                  handle.likeCounter.style.color = "green";
                else if(likeCounter<0)
                  handle.likeCounter.style.color = "red";
            }
        }
        ...
        if(!(document.getElementById("BigImg")))
            ...
            let topRated = document.createElement("topRated");
            let caption = document.createElement("p");
            topRated.innerText = "You Have the Highest Rated Drawing";
            caption.id = "caption";
            caption.innerText = data[0].name
            hold.appendChild(topRated);
            hold.appendChild(caption);
            let imgW = document.getElementById("image-wrapper");
            imgW.appendChild(img);
            imgW.appendChild(hold);
            img.style.borderColor = data[0].color;
            }
            else
            {
                let img = document.getElementById("image-wrapper");
                img.src = data[0].fileName;
                document.getElementById("caption").innerText = data[0].name;
                img.style.borderColor = data[0].color;
            }
        });
    ...
    $.get("/userInfo",function(data){
      username = data.name;
        if (data.name){
          ...
          $("#nameLabel").html(data.name + "'s GALLERY");
        }
